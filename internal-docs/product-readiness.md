...
| root + 平台 optional packages | 部分   | 8 个 binding 包和 8 个 BSD sidecar 包均有精确 optional dependency、文件白名单与版本门禁；当前 macOS ARM64 的 binding + cwebp + cjpeg + jpegtran tarball 已验证，尚缺 8 平台完整 artifact。 |
| 多平台二进制 CI               | 部分   | release workflow 已定义 8 target binding/cwebp/MozJPEG 构建、artifact 汇总及 GNU/musl/双架构 smoke；尚未在 release tag 上取得实跑证据。                                                    |
