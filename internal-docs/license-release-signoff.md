...
- 当前状态：**ENGINEERING PASS — L2 与 GPL 随包源码已由公开 RC 闭环**
...
3. 早期公开版本没有完成同版本闭环：`rc.6` 有 GPL sidecar 二进制、但 GitHub Release
   没有对应源码资产；`rc.7` 有源码资产、但 npm 没有同版本 GPL sidecar。`rc.9`
   已同时关闭这两个历史缺口。
4. pngquant 的构建会静态链接 Cargo 依赖；仅保存上游 pngquant、libimagequant 归档和
   `Cargo.lock` 不能单独覆盖 GPLv3 Corresponding Source。`rc.9` 的公开包含 45 个
   crates.io 源码归档；`fetch-pngquant-cargo-sources.mjs` 逐一下载并按 Cargo
   checksum 验证全部归档，不依赖主观排除 target-specific 项。
...
8. `v0.1.0-rc.9` 的每个 GPL 平台 tarball 包含对应上游源码、项目构建材料；pngquant
   额外包含 lockfile 中全部 45 个 registry 源码归档。manifest、pack verifier、
   八平台 Release smoke 和公开 npm registry 回读已核对版本、入口与摘要。
...
- [x] 全部 35 个 npm 包同版本公开，且 root optional dependency closure 可安装；
- [x] Git tag、GitHub Release、npm tarball 和 dist-tag 指向同一不可变版本；
- [x] GPL source-to-binary manifest 覆盖八平台及所有实际静态链接依赖；
- [x] `fetch-pngquant-cargo-sources.mjs` 取得的全部 lockfile registry 源码随所选交付
      模型发布，并由最终 tarball/release verifier 重新核对；
- [x] 最终 tarball 内的许可文本与固定官方摘要一致；
- [x] 八平台全新安装和全部 codec smoke 通过；
- [x] L1 不适用；L2 默认闭包不分发 Sharp；
- [x] 若选 L2，无 Sharp 安装与显式 Sharp 安装两种路径均在八平台通过；
- [x] 发布资产在 npm 与 immutable GitHub Release 双存储；公开资产摘要回读和恢复演练完成；
- [x] 审计人从公开 registry 重新下载并核对版本、摘要、notice 和源码入口。
...
维护者选择项已经关闭原来的“交付模型未决”HOLD；同一个完整 RC 的技术验证已全部
勾选，没有未记录例外，因此发行工程状态为 PASS。CI、registry 和恢复演练证明实现与
制品事实，不应被表述为法律意见。
