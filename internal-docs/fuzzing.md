...
| 2026-07-27 | `svg_pipeline` | 4-byte `<svg` (truncated document)                                     | SVGM serializes a rootless parse to an empty string, silently turning an SVG into a zero-byte non-SVG buffer. The adapter now fails with `ERR_IMAGEMIN_CODEC` when output stops being a detectable SVG, and `looks_like_svg` learned to skip XML comment prologues so preserved-comment output keeps its format. Regression tests `rejects_documents_that_stop_being_svg_after_optimization` and the format detection cases. |
